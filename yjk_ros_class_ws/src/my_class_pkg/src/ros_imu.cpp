...
void imu_callback(const sensor_msgs::Imu::ConstPtr& imu_msg) {
    ...
    // 打印IMU 数据
    ROS_INFO("Linear acceleration: x=%f, y=%f, z=%f", linear_acceleration.x,
    linear_acceleration.y, linear_acceleration.z);
    ROS_INFO("Angular velocity: x=%f, y=%f, z=%f", angular_velocity.x,
    angular_velocity.y, angular_velocity.z);
    ROS_INFO("Orientation: x=%f, y=%f, z=%f, w=%f", orientation.x, orientation.y,
