...
class SpeechRecognition:
    ...
    def __init__(self):
        rospy.init_node('robot_voice_speech_node', anonymous=True)  # 初始化ros节点
        self.service = rospy.Service("/start_recognize", Empty, self.handle_start_service) # 接收这个服务请求，可以开始语音识别
        self.speech_result_pub = rospy.Publisher('/speech/result',String, queue_size = 10)  # 语音识别的结果，发布在这个节点
        ...
        self.recognizer = self.create_recognizer() # 创建一个语音识别器
        print("Started! Please speak")
        self.stream = self.recognizer.create_stream() # 创建一个音频流
        self.asr_result = "" 
        self.start()
