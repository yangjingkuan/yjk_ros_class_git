...
void ControlCenter::initROSModule()
{
    ...
    // 初始化打开夹爪，末端水平运动service
    arm_pos_horizental_open_claw_service = nn.advertiseService("arm_pos_hor_service_open", &ControlCenter::arm_position_horizental_open_claw_callback, this);

    // 初始化闭合夹爪，末端水平运动service
    arm_pos_horizental_close_claw_service = nn.advertiseService("arm_pos_hor_service_close", &ControlCenter::arm_position_horizental_close_claw_callback, this);
...
//打开夹爪,末端水平,机械臂运动请求：传入位置参数，单位cm，左x正，前y正，上z正，控制机械臂运动，如果 resp.status 返回0，表示超出机械臂运动范围，返回1表示可以运动
bool ControlCenter::arm_position_horizental_open_claw_callback(upros_message::ArmPosition::Request &req, upros_message::ArmPosition::Response &resp)
{
    float x = req.x;
    float y = req.y;
    float z = req.z;
    ROS_INFO("arm position controller :x = %.2f, y = %.2f,z = %.2f", x, y, z);
    upros_arm.inverseHorizentalFind(x, y, z);
    upros_arm.inverseMoveToGrab();
    return true;
}

//闭合夹爪,末端水平,机械臂运动请求：传入位置参数，单位cm，左x正，前y正，上z正，控制机械臂运动，如果 resp.status 返回0，表示超出机械臂运动范围，返回1表示可以运动
bool ControlCenter::arm_position_horizental_close_claw_callback(upros_message::ArmPosition::Request &req, upros_message::ArmPosition::Response &resp)
{
    float x = req.x;
    float y = req.y;
    float z = req.z;
